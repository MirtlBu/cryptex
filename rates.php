<div class="background" style="height: 570px;"></div>
<div class="container-1230px">
    <div class="container-820px">
        <div class="rates" style="min-height: 910px;">
            <h1>Rates</h1>
            <div class="padded-field">
                <p class="text-greenfield">Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore
                magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea.</p>
                <div class="whitefield">
                    <table class="table">
                        <thead>
                            <tr>
                                <th class="th tcell--350px">Exchange</th>
                                <th class="th tcell--50px">Free</th>
                                <th class="th tcell--150px">Min Commission</th>
                                <th class="th tcell--100px">Type</th>
                                <th class="th tcell--150px">Reserve</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                <td class="td">Bitcoin BTC  - WesterUnion USD</td>
                                <td class="td">3%</td>
                                <td class="td">20 USD</td>
                                <td class="td">Manual</td>
                                <td class="td"> &gt; 5 000 usd</td>
                            </tr>
                            <tr>
                                <td class="td">Bitcoin BTC  - Contact USD</td>
                                <td class="td">5%</td>
                                <td class="td">30 USD</td>
                                <td class="td">Manual</td>
                                <td class="td"> &gt; 3 000 usd</td>
                            </tr>
                            <tr>
                                <td class="td">Bitcoin BTC  - WesterUnion USD</td>
                                <td class="td">3%</td>
                                <td class="td">20 USD</td>
                                <td class="td">Manual</td>
                                <td class="td"> &gt; 5 000 usd</td>
                            </tr>
                            <tr>
                                <td class="td">Bitcoin BTC  - Contact USD</td>
                                <td class="td">5%</td>
                                <td class="td">30 USD</td>
                                <td class="td">Manual</td>
                                <td class="td"> &gt; 3 000 usd</td>
                            </tr>
                            <tr>
                                <td class="td">Bitcoin BTC  - WesterUnion USD</td>
                                <td class="td">3%</td>
                                <td class="td">20 USD</td>
                                <td class="td">Manual</td>
                                <td class="td"> &gt; 5 000 usd</td>
                            </tr>
                            <tr>
                                <td class="td">Bitcoin BTC  - WesterUnion USD</td>
                                <td class="td">3%</td>
                                <td class="td">20 USD</td>
                                <td class="td">Manual</td>
                                <td class="td"> &gt; 5 000 usd</td>
                            </tr>
                            <tr>
                                <td class="td">Bitcoin BTC  - Contact USD</td>
                                <td class="td">5%</td>
                                <td class="td">30 USD</td>
                                <td class="td">Manual</td>
                                <td class="td"> &gt; 3 000 usd</td>
                            </tr>
                            <tr>
                                <td class="td">Bitcoin BTC  - WesterUnion USD</td>
                                <td class="td">3%</td>
                                <td class="td">20 USD</td>
                                <td class="td">Manual</td>
                                <td class="td"> &gt; 5 000 usd</td>
                            </tr>
                            <tr>
                                <td class="td">Bitcoin BTC  - WesterUnion USD</td>
                                <td class="td">3%</td>
                                <td class="td">20 USD</td>
                                <td class="td">Manual</td>
                                <td class="td"> &gt; 5 000 usd</td>
                            </tr>
                            <tr>
                                <td class="td">Bitcoin BTC  - Contact USD</td>
                                <td class="td">5%</td>
                                <td class="td">30 USD</td>
                                <td class="td">Manual</td>
                                <td class="td"> &gt; 3 000 usd</td>
                            </tr>
                            <tr>
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
    </div>
</div>
